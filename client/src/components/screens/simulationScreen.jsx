...
export default function SimulationScreen() {
  ...
  return (
    <div className="w-screen h-screen bg-light-space-gray flex flex-col gap-2 p-12">
      ...
      >
        <span
          className={`border-3 text-gray-400 border-orange-500 border-opacity-60 rounded-lg absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 p-4 z-50 text-xl text-nowrap font-bold group-hover:scale-105 group-hover:text-white group-hover:border-opacity-100 transition-all duration-300 text-center ${spanVisible ? "opacity-100" : "opacity-0"}`}
        >
          Load Simulation
        </span>
        <iframe
          src="https://eyes.nasa.gov/apps/solar-system/#/jupiter"
          className={`w-full h-full transition-all duration-300 ${loadSimulation ? "blur-none" : "blur-sm"}`}
